...
namespace HWindowView.Model
{
    ...
    /// shaped ROI. ROIRectangle1 inherits from the base class ROI and implements (besides other
    /// auxiliary
    /// methods) all virtual methods defined in ROI.cs. Since a simple rectangle is defined by two
    /// data points, by the upper left corner and the lower right corner, we use four values
    /// (row1/col1) and (row2/col2) as class members to hold these positions at any time of the
    /// program. The four corners of the rectangle can be taken as handles, which the user can use
    /// to manipulate the size of the ROI. Furthermore, we define a midpoint as an additional
    /// handle, with which the user can grab and drag the ROI. Therefore, we declare NumHandles to
    /// be 5 and set the activeHandle to be 0, which will be the upper left corner of our ROI.
    /// </summary>
    public class ROIRectangle1 : ROI
    {
        ...
        {
            pNumHandles = 5; // 4 corner points + midpoint
            pActiveHandleIdx = 4;
        }
        ...
        public override void Draw( HalconDotNet.HWindow window )
        {
            ...
            window.DispRectangle2( row1 , col2 , 0 , pSize.Width , pSize.Height );
            window.DispRectangle2( row2 , col2 , 0 , pSize.Width , pSize.Height );
            window.DispRectangle2( row2 , col1 , 0 , pSize.Width , pSize.Height );
